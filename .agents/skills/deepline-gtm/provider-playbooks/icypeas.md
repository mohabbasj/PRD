...
- **Verify before sending.** Always run `icypeas_email_verification` on discovered emails before outbound. It costs only 0.1 Icypeas provider credits. Treat `NOT_FOUND` and `DEBITED_NOT_FOUND` as non-deliverable.
